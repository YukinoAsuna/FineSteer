...
class OpenGenEvalPipeline:
    ...
    def flow_eval_pipeline(self, flow, wrapper, v, alpha, beta, eval_method:str="gpt", file_name:str="result"):
        """evaluate TruthFlow"""
        if eval_method == "gpt":
        ...
        for data in tqdm(self.test_ds):
            ...
            for idx, layer in enumerate(self.layers):
                ...
                self.model.model.layers[layer] = wrapper(self.model.model.layers[layer], hs_flow[0], v.to(self.device), k=self.k, alpha=alpha, beta=beta)
            self.model.eval()
